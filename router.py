...
async def get(db: Session = Depends(get_db)):
    ...
    return Response(code=200, status="ok", message="Data fetched successfully!!").dict(exclude_none=True)


@router.get('/{id}')
async def get_by_id(id: int, db: Session = Depends(get_db)):
    _book = crud.get_book_by_id(db, id)
    return Response(code=200, status="ok", message="Success get data").dict(exclude_none=True)
...
async def update_book(request: RequestBook, db: Session = Depends(get_db)):
    ...
    return Response(code=200, status="ok", message="updated successfully").dict(exclude_none=True)
